...
function AdminDashboardContent() {
  ...
  const fetchData = async () => {
    if (user) {
      ...
      try {
        ...
        // Fetch real dashboard statistics from database with timeout
        const [statsRes, coursesRes, enrollmentRequestsRes] = await Promise.allSettled([
          fetchWithTimeout('/api/real-stats'),
          fetchWithTimeout('/api/real-courses'),
          fetchWithTimeout('/api/real-enrollment-requests')
        ]);
        ...
        if (statsRes.status === 'fulfilled' && statsRes.value.ok) {
          ...
          console.warn('Failed to fetch stats:', statsRes.status === 'rejected' ? statsRes.reason : 'Response not ok');
        }
        ...
        if (coursesRes.status === 'fulfilled' && coursesRes.value.ok) {
          ...
          console.warn('Failed to fetch courses:', coursesRes.status === 'rejected' ? coursesRes.reason : 'Response not ok');
          setCourses([]); // Set empty array to prevent loading state
        }

        // Handle enrollment requests response
        if (enrollmentRequestsRes.status === 'fulfilled' && enrollmentRequestsRes.value.ok) {
          const enrollmentData = await enrollmentRequestsRes.value.json();
          setEnrollmentRequests(enrollmentData.requests || []);
        } else {
          console.warn('Failed to fetch enrollment requests:', enrollmentRequestsRes.status === 'rejected' ? enrollmentRequestsRes.reason : 'Response not ok');
        }

        // Fetch users data with timeout
        try {
          const usersRes = await fetchWithTimeout('/api/real-users');
          if (usersRes.ok) {
        ...
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Failed to load dashboard data. Some data may be unavailable.',
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleUserCreated = () => {
    ...
    // Refresh dashboard data using the same fetchData function
    fetchData();
  };
